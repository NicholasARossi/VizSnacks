...
svg.append("g")
  ...
  .append("text") // just for the title (ticks are automatic)
    ...
    .text("Total Delayed Flights (over last 2 weeks)");

var title_text=svg.append("text")
    .attr("class", "title")
    .attr("x",width/2-450)
    .attr("y",-20)
    .text('Top 10 Worst Airports by Total Delayed')
    .attr("font-size", "20px")
    .style("opacity", 1.0);
...
function draw(data) {
  ...
  bars.transition().duration(300*timemod).attr("x", function(d) { return x(d.airport); }) // (d) is one item from the data array, x is the scale object from above
    ...
    .attr("height", function(d) { return height - y(d.late); }); // flip the height, because y's domain is bottom up, but SVG renders top down
